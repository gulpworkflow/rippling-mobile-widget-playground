...
const TaskBadgePill = styled.div`
  display: flex;
  align-items: center;
  background: color-mix(in srgb, ${({ theme }) => (theme as StyledTheme).colorSurfaceDim} 50%, transparent);
  border-radius: ${({ theme }) => (theme as StyledTheme).shapeCornerMd};
  padding: 2px 12px 2px 2px;
`;

const TaskBadgeDivider = styled.div`
  width: 1px;
  height: 16px;
  background: ${({ theme }) => (theme as StyledTheme).colorOutlineVariant};
  flex-shrink: 0;
...
const TODO_CATEGORIES: TodoCategory[] = [
  { label: 'Approvals', count: 8, icon: Icon.TYPES.CHECKBOX_WITHCHECK_OUTLINE },
  { label: 'Tasks', count: 5, icon: Icon.TYPES.TASKS_OUTLINE },
];

const APPROVAL_SUBCATS = [
  { label: 'HR Management', count: 3 },
  { label: 'Time & Attendance', count: 2 },
  { label: 'Reimbursements', count: 3 },
];

const TASK_SUBCATS = [
  { label: 'Documents', count: 2 },
  { label: 'Learning Management', count: 2 },
  { label: 'Payroll', count: 1 },
];

const TaskBadgeWrap = styled.div`
  position: relative;
`;

const TodoHoverCard = styled.div<{ $visible: boolean }>`
  position: absolute;
  top: calc(100% + 8px);
  right: 0;
  width: 320px;
...
const TodoHoverHeader = styled.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 0 ${({ theme }) => (theme as StyledTheme).space400} ${({ theme }) => (theme as StyledTheme).space300};
  border-bottom: 1px solid ${({ theme }) => (theme as StyledTheme).colorOutlineVariant};
  margin-bottom: ${({ theme }) => (theme as StyledTheme).space200};
`;

const TodoHoverTitle = styled.span`
  ${({ theme }) => (theme as StyledTheme).typestyleV2LabelLarge};
  color: ${({ theme }) => (theme as StyledTheme).colorOnSurface};
`;

const TodoHoverCount = styled.span`
  ${({ theme }) => (theme as StyledTheme).typestyleV2BodySmall};
  color: ${({ theme }) => (theme as StyledTheme).colorOnSurfaceVariant};
`;

const TodoCategoryRow = styled.div`
  display: flex;
  align-items: center;
  gap: ${({ theme }) => (theme as StyledTheme).space300};
...
const TodoCatIcon = styled.div<{ $overdue?: boolean }>`
  width: 28px;
  height: 28px;
  border-radius: ${({ theme }) => (theme as StyledTheme).shapeCornerFull};
...
const TodoCatLabel = styled.span`
  ${({ theme }) => (theme as StyledTheme).typestyleV2LabelLarge};
  color: ${({ theme }) => (theme as StyledTheme).colorOnSurface};
  flex: 1;
`;

const TodoCatCount = styled.span<{ $overdue?: boolean }>`
  ${({ theme }) => (theme as StyledTheme).typestyleV2LabelMedium};
  color: ${({ $overdue, theme }) =>
    $overdue ? (theme as StyledTheme).colorError : (theme as StyledTheme).colorOnSurfaceVariant};
  min-width: 20px;
  text-align: right;
`;

const TodoSubcatRow = styled.div`
  display: flex;
  align-items: center;
  padding: 4px ${({ theme }) => (theme as StyledTheme).space400} 4px 52px;
  cursor: pointer;
...
const TodoSubcatLabel = styled.span`
  ${({ theme }) => (theme as StyledTheme).typestyleV2BodySmall};
  color: ${({ theme }) => (theme as StyledTheme).colorOnSurfaceVariant};
  flex: 1;
`;

const TodoSubcatCount = styled.span`
  ${({ theme }) => (theme as StyledTheme).typestyleV2BodySmall};
  color: ${({ theme }) => (theme as StyledTheme).colorOnSurfaceVariant};
  min-width: 20px;
  text-align: right;
...
const TodoHoverFooter = styled.div`
  border-top: 1px solid ${({ theme }) => (theme as StyledTheme).colorOutlineVariant};
  margin-top: ${({ theme }) => (theme as StyledTheme).space200};
  padding: ${({ theme }) => (theme as StyledTheme).space300} ${({ theme }) => (theme as StyledTheme).space400} 0;
`;

const TodoHoverViewAll = styled.button`
  ${({ theme }) => (theme as StyledTheme).typestyleV2LabelMedium};
  color: ${({ theme }) => (theme as StyledTheme).colorPrimary};
  background: none;
  border: none;
...
const PromptDropdown = styled.div<{ $visible: boolean }>`
  ...
  border: 1px solid ${({ theme }) => (theme as StyledTheme).colorOutlineVariant};
  border-top: none;
  border-radius: 0 0 ${({ theme }) => (theme as StyledTheme).shapeCorner3xl} ${({ theme }) => (theme as StyledTheme).shapeCorner3xl};
...
const DesktopHome422Shippable: React.FC = () => {
  ...
  const handleTodoHoverEnter = () => {
    if (todoHoverTimeout.current) clearTimeout(todoHoverTimeout.current);
    setTodoHoverOpen(true);
  };

  const handleTodoHoverLeave = () => {
    todoHoverTimeout.current = setTimeout(() => setTodoHoverOpen(false), 150);
  };
